...
resource "azurerm_resource_group" "gitvm1" {
...
#    resource_group_name = azurerm_resource_group.gitvm1.name
#    location = azurerm_resource_group.gitvm1.location
#    sku_name = "standard"
...
resource "azurerm_virtual_network" "vngitvm1" {
...
resource "azurerm_subnet" "sbgitvm1" {
    name = "${var.resource_group_name}-SUBNET-back"
    resource_group_name = azurerm_resource_group.gitvm1.name
    virtual_network_name = azurerm_virtual_network.vngitvm1.name
    address_prefixes = ["10.0.1.0/24"]
}

resource "azurerm_subnet" "sb2gitvm1" {
...
resource "azurerm_network_security_group" "nsggitvm1" {
    ...
    security_rule {
        ...
        destination_address_prefix = azurerm_network_interface.nigitvm1.private_ip_address
        }
    security_rule {
        name = "allow-ssh"
        priority = 101
        ...
        destination_address_prefixes = azurerm_subnet.sbgitvm1.address_prefixes
    }
}

# Creo la ip publica
resource "azurerm_public_ip" "pigitvm1" {
...
resource "azurerm_network_interface" "nigitvm1" {
    name = "${var.resource_group_name}-NIC"
    location = azurerm_resource_group.gitvm1.location
    resource_group_name = azurerm_resource_group.gitvm1.name
    ip_configuration {
      name = "${var.resource_group_name}-IP-NIC"
      subnet_id = azurerm_subnet.sbgitvm1.id
      private_ip_address_allocation = "Static"
      private_ip_address = "10.0.1.5"
      public_ip_address_id = azurerm_public_ip.pigitvm1.id
      } 
}
resource "azurerm_network_interface" "ni2gitvm1" {
    name = "${var.resource_group_name}-NIC2"
    location = azurerm_resource_group.gitvm1.location
    resource_group_name = azurerm_resource_group.gitvm1.name
    ip_configuration {
      name = "${var.resource_group_name}-IP-NIC"
      subnet_id = azurerm_subnet.sb2gitvm1.id
      private_ip_address_allocation = "Static"
...
resource "azurerm_network_interface_security_group_association" "nisggitvm1" {
    network_interface_id = azurerm_network_interface.nigitvm1.id
    network_security_group_id = azurerm_network_security_group.nsggitvm1.id
...
module "vm" {
  ...
  id_interface = azurerm_network_interface.nigitvm1.id
}
...
module "vm2" {
  ...
  publickey = var.publickey
  passvm = random_password.name.result
  id_interface = azurerm_network_interface.ni2gitvm1.id
}
...
#   name                       = "OmsAgentForLinux"
#   virtual_machine_id         = azurerm_virtual_machine.gitvm1vm2.id
#   publisher                  = "Microsoft.EnterpriseCloud.Monitoring"
#   type                       = "OmsAgentForLinux"
#   type_handler_version       = "1.13"
#   auto_upgrade_minor_version = true
...
#   resource_group_name = azurerm_resource_group.gitvm1.name
#   scopes              = [azurerm_virtual_machine.gitvm1vm.id, azurerm_virtual_machine.gitvm1vm2.id]
...
#   target_resource_location = azurerm_resource_group.gitvm1.location
# }
...
output "public_ip_address" {
  value = azurerm_public_ip.pigitvm1.ip_address
